    ...
    struct libusb_transfer *libusb_transfer)
{
    ...
    struct usbredirtransfer *transfer = libusb_transfer->user_data;
    struct usbredirhost *host = transfer->host;
    // fprintf(stderr, "%s--%d transfer->actual_length = %d transfer->status = %d \n",
    ...
    // fprintf(stderr, "%s--%d id %d cancelled = %d\n", __FUNCTION__, __LINE__, transfer->id, !transfer->cancelled);
    if (!transfer->cancelled) {
    ...
    uint8_t *data, int data_len)
{
    ...
    // fprintf(stderr, "control submit ep %02X len %d id %d\n"PRIu64, ep,
    //       control_packet->length, id);
    if (host->disconnected) {
    ...
    // fprintf(stderr, "%s--%d length = %d\n", __FUNCTION__, __LINE__, transfer->transfer->actual_length);
    r = libusb_submit_transfer(transfer->transfer);
